...
class Middle extends Component{
  ...
  componentDidMount(){
    ...
    updateOnClick = async (event)=>{
        ...
        //
        // this.setState({loader: !this.state.loader});
        //SHOW BUTTONS TO RATHER RUN ALGORITHM OR CANCEL FILE
    ...
    return (
      <div className="manip__container">
        ...
        <div onClick={this.removeFilesFromStorage} style={this.state.algorithm ? {display:"flex"} : {display:"none"} }className="update__bottom">
          ...
          {/*LOADER*/}
          <div className="loader__screen" style={(this.state.loader || this.state.algorithm) ? {display:"flex"} : {display:"none"} }>
            <svg style={ !this.state.algorithm ? {display:"block"} : {display:"none"} } className="loader__img" version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
